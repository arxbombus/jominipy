"""Modern Jomini lexer for v2 pipeline."""
...
class TokenType(Enum):
    ...
    NUMBER = auto()
    PERCENTAGE = auto()
    DOUBLE_PERCENTAGE = auto()  # Yeah...Whatever, the name sucks, idc
    BOOLEAN = auto()
    OPERATOR = auto()
    COMMENT = auto()
    EOF = auto()
...
class LexerMetadata:
    ...
    @classmethod
    def from_iterables(
        cls, *, keywords: Iterable[str] = (), modifiers: Iterable[str] = (), triggers: Iterable[str] = ()
    ):
        return cls(set(keywords), set(modifiers), set(triggers))


class JominiLexer:
    def __init__(self, text: str, metadata: LexerMetadata | None = None, *, capture_comments: bool = False):
        self.text = text
    ...
    def _consume_comment(self) -> None:
        ...
        comment_text = "".join(buffer)
        if self._capture_comments and comment_text:
    ...
    def _emit_number(self) -> None:
        ...
        if not self._is_eof and (self._peek().isalnum() or self._peek() in IDENTIFIER_EXTRA_CHARS):
            ...
            return
        if not self._is_eof and self._peek() == "%":
            self._emit_percentage(buffer, start_line, start_col)
            return
        text = "".join(buffer)
    ...
    # Processes percentages like 10%, 10.5%, 10%%
    def _emit_percentage(self, buffer: list[str], start_line: int, start_col: int) -> None:
        buffer.append(self._advance())  # consume '%'
        if not self._is_eof and self._peek() == "%":
            buffer.append(self._advance())
        text = "".join(buffer)
        dot_count = text.count(".")
        if dot_count > 1:
            ...
            return
        value: float
        value = float(text) if dot_count == 1 else float(int(text.strip("%")))
        self.tokens.append(
            Token(
                TokenType.DOUBLE_PERCENTAGE if text.endswith("%%") else TokenType.PERCENTAGE,
                value,
                start_line,
                start_col,
            )
...
__all__ = ["JominiLexer", "LexerMetadata", "Token", "TokenType"]
