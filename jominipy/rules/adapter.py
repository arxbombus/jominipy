...
def build_complex_enum_definitions(schema: RuleSchemaGraph) -> dict[str, tuple[ComplexEnumDefinition, ...]]:
    ...
    for declaration in schema.by_category.get("complex_enum", ()):
        ...
        if statement.value.kind != "block":
            continue
        paths = tuple(path for path in _find_scalar_children(statement.value.block, "path") if path)
        path_strict = _find_scalar_child(statement.value.block, "path_strict") == "yes"
...
def _find_scalar_children(statements: tuple[RuleStatement, ...], key: str) -> tuple[str, ...]:
    values: list[str] = []
    for statement in statements:
        if statement.kind != "key_value" or statement.key != key:
            continue
