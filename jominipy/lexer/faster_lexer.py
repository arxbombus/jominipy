...
class Lexer:
    ...
    def next_token(self) -> Token:
        ...
        self._current_start = TextSize.from_int(position)
        self._current_flags = TokenFlags.NONE
        ...
        if self._after_newline:
            self._current_flags |= TokenFlags.PRECEDING_LINE_BREAK
        ...
        return Token(
            kind,
            TextRange.new(self._current_start, TextSize.from_int(self._position)),
            self._current_flags,
        )
    ...
    def _lex_token(self) -> TokenKind:
        ...
        peek = source[position + 1] if (position + 1) < source_length else "\0"
        if ch == "=" and peek == "=":
    ...
    def _lex_number(self) -> TokenKind:
        ...
        while position < source_length:
            ...
            if ch.isdigit():
                position += 1
                continue
            if ch == "." and not saw_dot and (position + 1) < source_length and source[position + 1].isdigit():
                saw_dot = True
                position += 1
                continue
            break
    ...
    def _consume_newline(self) -> bool:
        ...
        if current == "\n":
            ...
            return True
        if current == "\r":
            if (position + 1) < source_length and source[position + 1] == "\n":
                self._position = position + 2
            else:
                self._position = position + 1
            return True
